...
if [ "$EXECUTION" = "docker" ]; then
    ...
    xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24" mvn test -Dbase.url="${BASE_URL}" -Dbrowser="${BROWSER}" -Denvironment="${ENVIRONMENT}" -Dexecution="${EXECUTION}" -Dretry.count="${RETRY_COUNT}" -Dretry.delay.ms="${RETRY_DELAY_MS}"
else
    mvn test -Dbase.url="${BASE_URL}" -Dbrowser="${BROWSER}" -Denvironment="${ENVIRONMENT}" -Dexecution="${EXECUTION}" -Dretry.count="${RETRY_COUNT}" -Dretry.delay.ms="${RETRY_DELAY_MS}"
fi