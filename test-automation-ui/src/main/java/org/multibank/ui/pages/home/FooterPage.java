...
public class FooterPage extends BasePage {
    ...
    public boolean isBannerSliderVisible() {
        return actions.isLocatorVisible(page.locator(BANNER_SLIDER).first());
    }

    @Step("Get banner count")
    public int getBannerCount() {
        return page.locator(BANNER_SLIDE).count();
    }

    @Step("Check if banner at index {index} is visible")
    public boolean isBannerVisible(int index) {
        return actions.isLocatorVisible(page.locator(BANNER_SLIDE).nth(index));
    }

    @Step("Scroll to the footer section")
    public void scrollToFooter() {
        page.locator(DOWNLOAD_CONTAINER).first().scrollIntoViewIfNeeded();
    }

    @Step("Get App Store link href")
    public String getAppStoreLinkHref() {
        return page.locator(APP_STORE_LINK).first().getAttribute("href");
    }

    @Step("Get Google Play link href")
    public String getGooglePlayLinkHref() {
        return page.locator(GOOGLE_PLAY_LINK).first().getAttribute("href");
    }
}